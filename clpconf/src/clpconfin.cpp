		...
		for (i = 0; i < length; i++)
		{
			...
			if (strlen(attribute) > 0)
			{
				...
				hr = child_elem->getAttribute(
					_bstr_t((strcmp(element, "device") == 0 || strcmp(element, "list") == 0 || strcmp(element, "hba") == 0) ? "id" : "name"), &attr);
	...
	try
	{
		...
		if (strlen(attribute) > 0)
		{
			...
			hr = child_elem->setAttribute(
				_bstr_t((strcmp(element, "device") == 0 || strcmp(element, "list") == 0 || strcmp(element, "hba") == 0) ? "id" : "name"), var);
